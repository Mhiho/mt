...
async function authenticate(body) {
  ...
  await db.User.update({ tokens: user.tokens }, { where: { userID: user.userID } }).catch((e) =>
    console.log(e),
  );
...
async function verify(token) {
  ...
  const tokenTrue = jwt.sign({ id: user.userID, valid: user.validated }, secret, {
    expiresIn: '1d',
  });
  ...
  if (user.validated === false) {
    await db.User.update(
      { validated: true, avatar: 'avatars/fighter.jpg' },
...
async function checkResetMail(mailToken) {
  ...
  const token = await db.Token.findOne({ where: { token: mailToken, userID: decoded.id } }).catch(
    (e) => console.log(e),
  );
  if (token) {
    return token;
...
async function resetPwd(password, token) {
  ...
  await db.User.update({ passwordHash: reseted }, { where: { userID: decoded.id } }).catch((e) =>
    console.log(e),
  );
...
function omitHash(user) {
  ...
  const { passwordHash, userID, tokens, email, ...cleanUser } = user;
